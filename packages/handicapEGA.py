...
def calculateNewHandicap(
    game: dict, cba: int, previousHandicap: float, course: dict
) -> float:
    """
    ...
    handicapStrokes = playingHandicap(
        game["is9Hole"],
        previousHandicap,
...
def ganzzahligeDivision(dividend: float, divisor: float) -> float:
    if divisor == 0:
        raise ZeroDivisionError("Du kannst doch nicht einfach durch 0 teilen du hirni!!")
...
def convertToStableford(shots: list[int], adjustedPar: list[int]) -> int:
    ...
    """
    if len(shots) > len(adjustedPar):
        raise ValueError("List of shots taken cannot be longer than the list of Par.")
    score = 0
...
def playingHandicap18(
    handicap: float, courseRating: float, slopeRating: float, par: int
) -> int:
    """
    ...
    # category 6
        ...
        raw = handicap + playingHandicapDifferential(
            False, courseRating, slopeRating, par
        )
...
def playingHandicap9(
    ...
    if category == 6:
        raw = handicap / 2 + playingHandicapDifferential(
            True, courseRating, slopeRating, par
        )
...
def playingHandicap(
    is9Hole: bool, handicap: float, courseRating: float, slopeRating: float, par: int
) -> int:
    """
    ...
    if not (55 <= slopeRating <= 155):
        raise ValueError(
            f"Slope rating must be between 55 and 155, but was {slopeRating}"
        )
...
def playingHandicapDifferential(
    nineHole: bool, courseRating: float, slopeRating: float, par: int
) -> float:
    """
...
def calculateAdjustment(
    stablefordScore: int, handicap: float, cba: int, is9Hole: bool
) -> float:
    """
